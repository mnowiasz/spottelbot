...
def test_loadconfig():
    _controller.config.load_config(open(_config_file_valid))
...
def test_loadconfig_missingsections(filename):
    ...
    with pytest.raises(botexceptions.MissingSection):
        _controller.config.load_config(open(config_file_missingsection))


def test_telegram_token():
    _controller.config.load_config(open(_config_file_valid))
    assert "110201543:AAHdqTcvCH1vGWJxfSeofSAs0K5PALDsaw" == _controller.config._telegram_token


def test_spotify_username():
    _controller.config.load_config(open(_config_file_valid))
    assert "SpotifyUser" == _controller.config._spotify_username
...
class TestAccessConfig(object):
    ...
    def setup_class(cls):
        ...
        cls._controller.config.load_config(open(_config_file_valid))
...
def test_loadconfig_missingusers(filename):
    ...
    with pytest.raises(botexceptions.MissingUsers):
        _controller.config.load_config(open(config_file_missingusers))
...
def test_loadconfig_badusers():
    ...
    with pytest.raises(botexceptions.InvalidUser) as invalid_exception:
        _controller.config.load_config(open(config_file_badusers))
...
def test_loadconfig_missingtoken(filename):
    ...
    with pytest.raises(botexceptions.MissingTelegramToken):
        _controller.config.load_config(open(config_file_missingtoken))
...
def test_loadconfig_missingusername(filename):
    ...
    with pytest.raises(botexceptions.MissingSpotifyUsername):
        _controller.config.load_config(open(config_file_missingusername))
...
def test_loadconfig_invalidbookmarks():
    ...
    with pytest.raises(botexceptions.InvalidBookmark) as invalid_bookmark:
        _controller.config.load_config(open(config_file_invalid))
...
def test_loadconfig_missingbookmarks():
    ...
    with pytest.raises(botexceptions.InvalidBookmark) as invalid_bookmark:
        _controller.config.load_config(open(config_file_missing))
    assert "current" == invalid_bookmark.value.invalid_bookmark


def test_duplicate_configread():
    test_file = open(_config_file_valid)
    _controller.config.load_config(test_file)
    _controller.config.load_config(test_file)
    _controller.config.load_config(test_file)
